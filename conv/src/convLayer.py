...
class ConvLayer(object):
    ...
    def forward(self, input):
        ...
        for f in range(0, len(self.filters)):
            ...
            filter_map = self.input_rolled_out * filter_reshaped
    ...
    def backward(self):
        ...
        for f in range(0, len(self.filters)):
            ...
            filter_dw = (filter_reshaped_dw + (out_grad_tiled * self.input_rolled_out))
            ...
            for index in range(0, self.input_rolled_out_indexes.shape[1]):
                current_grad = self.input.grads.take(self.input_rolled_out_indexes[:, index])
                input_grad = input_dw[:, index]
                numpy.put(self.input.grads, self.input_rolled_out_indexes[:, index], current_grad + input_grad)
