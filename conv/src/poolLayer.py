...
class PoolLayer(object):
    ...
    def __init__(self, stride, spatial_extent, name=""):
        self.stride = stride
    ...
    def forward(self, input_matrix):
        ...
        c = pooled_out.copy()
        c.fill(0)
        self.out = ConvMatrix(input_z, out_y, out_x, pooled_out, c)
        return self.out
    ...
    def backwards(self, y):
        ...
        for f in range(0, out_z):
            g = numpy.take(self.input.grads[f], self.pooled_out_max_indices[f]).reshape(int(out_y), int(out_x))
            chained = g + self.out.grads[f]
    ...
    def from_dict(cls, dict):
        obj = cls(dict['stride'], dict['spatial_extent'], dict['name'])
        return obj
